...
Tool assumes no interaction i.e. build-up factor is 1.
"""
import numpy as np
import pandas as pd
...
class RadiationZoneMapper:
    ...
    - Supervised Area: 0.5 - 3 µSv/hr
    - Controlled Area: 3 - 10 µSv/hr
    - Restricted Area: > 10 µSv/hr
   
    Note: Thresholds are approximate dose rate equivalents derived from annual effective dose limits
    (Non-designated <1 mSv/y, Supervised 1-6 mSv/y, Controlled 6-20 mSv/y, Restricted >20 mSv/y)
    assuming 2000 hours occupancy per year under normal working conditions. They apply to the total
    effective dose, including prompt and residual radiation.
    ...
    def create_interpolated_map(self, measurements_df, method='cubic',
                            resolution=200, buffer_distance=2):
        """
        ...
        method : str
            'idw' (mimics inverse squared pattern)
            'kriging' (provides uncertainty- can be used for field data)
            'linear', 'cubic', 'nearest' (scipy fallbacks)
        resolution : int
            Grid resolution for interpolation
        buffer_distance : float
            Distance (m) to extend beyond measurement points (Gaussian)
        Returns:
        --------
        tuple : (grid_x, grid_y, grid_dose, grid_variance)
            grid_variance is None when not using kriging
        """
        ...
        if method == 'idw':
            ...
            dists, indices = tree.query(grid_coords, k=12)
            ...
            grid_z = numerator / denominator
            
            grid_dose = grid_z.reshape(grid_x.shape)
            self._last_variance = None
            return grid_x, grid_y, grid_dose, None
        ...
        if method == 'kriging': 
            try:
                ...
                grid_x_1d = np.linspace(x_min, x_max, resolution)
                grid_y_1d = np.linspace(y_min, y_max, resolution)
                grid_dose_kriged, grid_variance = OK.execute('grid', grid_x_1d, grid_y_1d)
                grid_dose = np.array(grid_dose_kriged).T
                grid_variance = np.array(grid_variance).T
                grid_dose = np.clip(grid_dose, 0, None)
                self._last_variance = grid_variance
                return grid_x, grid_y, grid_dose, grid_variance
            except ImportError:
                print("pykrige not installed, falling back to linear")
                method = 'linear'
    ...
    def plot_uncertainty_map(self, grid_x, grid_y, grid_variance, figsize=(10, 7), area_name='Survey Area'):
        """
        Plot kriging variance map showing where zone boundaries are uncertain.
        Only available after create_interpolated_map() with method='kriging'.
        High variance = zone boundary here is uncertain, more measurements needed.
        Low variance = boundary is well-constrained by nearby measurements.
        """
        if grid_variance is None:
            print("No kriging variance available. Run create_interpolated_map with method='kriging' first.")
            return None
        fig, ax = plt.subplots(figsize=figsize)
        std_dev = np.sqrt(grid_variance)
        im = ax.contourf(grid_x, grid_y, std_dev, levels=20, cmap='YlOrRd')
        cbar = plt.colorbar(im, ax=ax, pad=0.02)
        cbar.set_label('Dose Rate Std Dev (µSv/hr)\n[Higher = more uncertain]',
        ...
        ax.set_title(f'Kriging Uncertainty Map\n{area_name}\n'
                     f'Regions in red need additional survey points',
    ...
    def plot_zone_map(self, measurements_df, area_name='Experimental Area',
                     ...
                     interp_method='cubic', adaptive_scale=True):
        """
        ...
        interp_method : str
            Interpolation method for map
        adaptive_scale : bool
            Whether to adaptively scale the color levels (not used in current implementation) #to be implemented to the tool
           
        Returns:
        --------
        matplotlib.figure.Figure
        """
        grid_x, grid_y, grid_dose, grid_variance = self.create_interpolated_map(measurements_df, method=interp_method)
       
        fig, ax = plt.subplots(figsize=figsize)
       
        max_level = max(grid_dose.max() * 1.1, self.zone_limits['Controlled'] * 1.1)
        levels = [0,
        ...
        if add_buffer_zones:
            ...
            if restricted_mask.any():
                dist = distance_transform_edt(~restricted_mask)
                buffer_mask = (dist > 0) & (dist <= 2 / ((grid_x[0,1] - grid_x[0,0])))  # Approx 2m in grid units
                ax.contour(grid_x, grid_y, buffer_mask,
                          levels=[0.5], colors='red',
                          linewidths=3, linestyles='--',
                          label='2m Buffer Zone')
    ...
    def calculate_shielding_required(self, current_dose, target_dose, material='concrete', energy_MeV=1.0):
        """
        Calculates required shielding thickness using: I = I0 * e^(-mu * x)
        Rearranged: x = -ln(I / I0) / mu
        Uses accurate mu values at 1.0 MeV from NIST XCOM.
        """
        ...
        mu = mu_values.get(material.lower(), 0.153)
...
def create_sample_data(scenario='beamline_hotspot'):
    ...
    if scenario == 'beamline_hotspot':
        ...
        x_coords, y_coords, dose_rates = [], [], []
        
        beam_y = 18.0
        hotspots = [(10, 25), (25, 45), (40, 22)]
        
        y_positions = [15.0, 16.0, 17.0, 18.0, 19.0, 20.0, 21.0]
        x_positions = np.linspace(0, 50, 12)  
        
        for x in x_positions:
            for y in y_positions:
                r = max(abs(y - beam_y), 1.0)
                base_dose = 8.0 / r
                
                hotspot_dose = 0
                for hx, intensity in hotspots:
                    dist = max(np.sqrt((x-hx)**2 + (y-beam_y)**2), 2.0)
                    hotspot_dose += intensity / dist**2
                
                dose = base_dose + hotspot_dose
                dose *= np.random.uniform(0.92, 1.08)
                dose = max(1.0, min(dose, 150))
...
if __name__ == "__main__":
    ...
    grid_x, grid_y, grid_dose, _ = mapper.create_interpolated_map(sample_data)
